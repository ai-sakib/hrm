<template>
    <div v-if="!isLoading">
        <form @submit.prevent="editMode ? updateOTLevel() : storeOTLevel()">
        <div class="my-3 flex justify-between">
            <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">
                <span v-show="!editMode">Create OT Level</span>
                <span v-show="editMode">Edit OT Level</span>
            </h2>
        ...
        <div class="grid grid-cols-3 gap-2 w-full px-2 py-3 mb-3 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <label class="w-full block text-xs">
                <span class="mb-2 text-gray-700 dark:text-gray-400">OT Level Name <span class="text-red-500">*</span></span>
                <input type="text" 
                  ...
                  placeholder="Name" v-model="otLevel.name" required
                />
            </label>
            
            <label class="w-full block text-xs">
                <span class="mb-2 text-gray-700 dark:text-gray-400">Hours <span class="text-red-500">*</span></span>
                <input type="number" 
                  class="w-full border-gray-200   rounded-md block mt-0 text-xs dark:border-gray-600 dark:bg-gray-700 focus:border-blue-300 focus:outline-none focus:shadow-outline-blue dark:text-gray-300 dark:focus:shadow-outline-gray"
                  placeholder="Hours" v-model="otLevel.hours"
                />
            </label>
        </div>
...
<script scoped>
    ...
    export default {
        ...
        methods: {
            getOTLevel(){
                axios.get('ot-levels/'+this.id)
                .then(response => {
                    this.isLoading = false
                    this.otLevel = response.data.otLevel
                })
            ...
            updateOTLevel(){
                this.$messages.confirm().then(isConfirmed => {
                    if(isConfirmed){
                        axios.put('ot-levels/'+this.id, this.otLevel)
                        .then(response => {
                            if(response.data.success){
                                this.$messages.success('Updated Successfully')
                                window.location.href="/ot-levels"
                            }
                        })
                        .catch(error => console.log(error))
                    }
            ...
            storeOTLevel(){
                this.$messages.confirm().then(isConfirmed => {
                    if(isConfirmed){
                        axios.post('ot-levels', this.otLevel)
                        .then(response => {
                            if(response.data.success){
